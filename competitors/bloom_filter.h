...
class BloomFilter {
 public:
  BloomFilter() : num_bits_(0), num_hashes_(0) {}
  ...
  void init(size_t expected_elements, double fpr = 0.01) {
    ...
    // Round up to nearest multiple of 64 (one uint64_t word).
    num_bits_ = (num_bits_ + 63) & ~size_t(63);
    // Optimal hash count: k = (m/n) * ln 2
    num_hashes_ = std::max(size_t(1),
        static_cast<size_t>(std::round((double)num_bits_ / expected_elements * std::log(2.0))));
